...
export const getRegistroSalida = async () => {
    const { rows } = await query(`
        SELECT S.*, V.modelo 
        FROM salidas_td S 
        INNER JOIN vehiculos_td V ON S.id_vehiculo_fk = V.id 
...
export const postRegistroSalida = async (registroDataSalida) => {
    ...
    const { rows: salidaRows } = await query(`
        INSERT INTO salidas_td (id_vehiculo_fk, nombre_motorista, fecha_salida, hora_salida, kilometraje_salida, status, created_at, updated_at)
        VALUES ($1, $2, CURRENT_DATE, CURRENT_TIME, $3, TRUE, NOW(), NOW())
        RETURNING *
    `, [id_vehiculo_fk, nombre_motorista, kilometraje_salida]);

    if (salidaRows.length === 0) throw new Error("No se pudo registrar la salida.");
...
export const putRegistroSalida = async (registroId, datosActualizar) => {
    ...
    const { rows } = await query(`
        UPDATE salidas_td 
        SET nombre_motorista = $1, kilometraje_salida = $2, id_vehiculo_fk = $3, updated_at = NOW()
        WHERE id = $4
        RETURNING *
...
export const deleteRegistroSalida = async (registroId) => {
    try {
        ...
        console.error("Error al eliminar registroSalida:", error);
        throw error;
    }
};
...
export const searchRegistroSalida = async (searchTerm) => {
    ...
    const { rows } = await query(`
        SELECT S.*, V.modelo AS modelo
        FROM salidas_td S
        INNER JOIN vehiculos_td V ON V.id = S.id_vehiculo_fk
        WHERE
